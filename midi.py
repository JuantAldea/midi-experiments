...
def io_main(input_id, output_id):
    pygame.init()
    ...
    midi_out = pygame.midi.Output(output_id, 0)
    ...
    while True:
        ...
        if i.poll():
            ...
            #pp(midi_events)
            #pp(midi_events[0][0][1])
	    if (midi_events[0][0][0] == 144):
	        midi_out.note_on(midi_events[0][0][1] - 12 * 1, midi_events[0][0][2])
#       	     midi_out.note_on(midi_events[0][0][1] - 12 * 2, midi_events[0][0][2])
	            #midi_out.note_on(midi_events[0][0][1] - 12 * 4, midi_events[0][0][2])
...
def input_main(device_id = None):
    ...
    while going:
        ...
        if i.poll():
            ...
            midi_evs = pygame.midi.midis2events(midi_events, i.device_id)
            pp(midi_events)
            plt_mgr.add("time", midi_events[0][0][1])
            plt_mgr.update()
            for m_e in midi_evs:
                event_post( m_e )
                pp(m_e)
...
def key_images():
    ...
    strip_file = os.path.join(my_dir, 'midikeys.png')
    white_key_width = 42
...
if __name__ == '__main__':
    ...
    if "--input" in sys.argv or "-i" in sys.argv:
        ...
        pp("IO")
        io_main(int(sys.argv[2]), int(sys.argv[3]))
    else:
        usage()
